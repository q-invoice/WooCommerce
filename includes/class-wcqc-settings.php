...
if ( ! class_exists( 'WooCommerce_Qinvoice_Connect_Settings' ) ) {
	...
	class WooCommerce_Qinvoice_Connect_Settings {
		...
		public function settings_page() {
			...
			$tab_array = array();
			$tab_array['general'] = __('General settings','wcqc');
			// $tab_array['wpml'] = __('WPML','wcqc');
			$tab_array['payments'] = __('Payment methods','wcqc');
			// $tab_array['Sync'] = __('Sync','wcqc');
				...
				<div class="wrap">
					...
					<form method="post" action="options.php">
						<h2 class="nav-tab-wrapper">
							<?php foreach($tab_array as $tab => $title): ?>
							    <a href="?page=wcqc_options_page&tab=<?php echo $tab; ?>" class="nav-tab <?php if($tab == $active_tab){ echo 'nav-tab-active'; } ?>">
							    	<?php echo $title; ?>
							    </a>
							<?php endforeach; ?>
						...
						<?php 
							if($active_tab == 'payments'){
								...
								$pmnt = new WC_Payment_Gateways();
								$available_gateways = $pmnt->get_available_payment_gateways();

								foreach($available_gateways as $gw){
									if(@$_GET['method'] == $gw->id){
										echo '<strong>'. $gw->title .'</strong> | ';
									}else{
										echo '<a href="?page=wcqc_options_page&tab=payments&method='. $gw->id .'">'. $gw->title .'</a> | ';
									}
								}

								if(array_key_exists(@$_GET['method'],$available_gateways)){
									// echo 'wcqc_payment_'. $_GET['method'];
									settings_fields( 'wcqc_payment_'. $_GET['method'] );
									do_settings_sections( 'wcqc_payment_'. $_GET['method'] );
									submit_button();
								}
		...
		function register_settings() {
			...
			foreach ( $available_gateways as $gw ) {
				...
				register_setting( $option, $option, 0 ); 
			}

			register_setting( 'wcqc_general_settings', 'wcqc_general_settings', array( &$this, 'validate_options' ) );
		} 
