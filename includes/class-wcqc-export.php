	...
	class WooCommerce_Qinvoice_Connect_Export {
		...
		public function send_request( $request_type = 'invoice', $order_id, $output = true )	{
			...
			try{
				...
				$payment_method_settings = get_option('wcqc_payment_'.$payment_method_id);
				
				if(is_array($payment_method_settings)){
					// set [request type] (preparation for later, move/copy document dropdown to payment settings)
				}
				...
				// override values if settings for payment method are in place
				
				if(is_array($payment_method_settings)){
						
					if($payment_method_settings['invoice_trigger'] == 'none'){
						return true;
					}

					$document->action = (int)$payment_method_settings['invoice_action'];
					
					if($payment_method_settings['send_payment_status'] == 'no'){
						 $document->paid = 0;
					}else{
						$paid_date = get_post_meta($order_id,'_paid_date', true);
						$document->paid = 1;	
						$document->addTag($paid_date);
					}
